...
`
  ...
  function animate() {
      ...
      let s = new THREE.Matrix4().makeScale(1.5, 0.1, 1.5)
  ...
  gui.add(controls, 's_x').min(.1).max(2).onChange(animate)
  gui.add(controls, 's_y').min(1).max(2).onChange(animate)
  gui.add(controls, 's_z').min(1).max(2).onChange(animate)
...
export function displayHalfSphere() {
  const vs_script = `#version 300 es
  in vec3 coordinates;
  in vec3 color;
  out vec4 vColor;
...
`
  ...
  let sphere = new HalfSphere(.9, 69)
