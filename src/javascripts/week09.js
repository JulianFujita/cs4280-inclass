...
export function displayLightedScene() {
  ...
  geometry = new THREE.SphereBufferGeometry(25, 30, 30)
  let sphere = new THREE.Mesh(geometry)
  sphere.materialParams = { color: 0xFF0000 }
  sphere.position.set(-100, 50, 50)
  scene.add(sphere)

  geometry = new THREE.TorusBufferGeometry(30, 10, 25, 25)
  let torus = new THREE.Mesh(geometry)
  torus.materialParams = { color: 0x0000FF}
  ...
  f.add(controls, 'theta').min(-1 * Math.PI).max(Math.PI)
  f.add(controls, 'phi').min(-1 * Math.PI).max(Math.PI)
  f.open()
